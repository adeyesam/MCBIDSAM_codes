...
function y = simulate_model_dynamic2(inputs, y_initial, solnrank,flag)
...
for i=1:N
    y_initial_(i,:)=1 + 2*(y_initial(i,:)-mnd(i))/(mxd(i)-mnd(i));
end
...
for jj=1:lengthu2
    ...
    A = reshape(Am,[N,N])';
    C1 = reshape(C1m,[lengthu,N])';
    C2_ = reshape(C2m,[Ntrans,N])';
    C2 = C2_(:,indx);      
    D = Dm';
    ...
    % if jj==1
    %     XQ(:,jj+1) = A*XXQ + C1*u(:,jj) + C2*Utrans_ + D;
    % else
        XQ(:,jj+1) = A*XQ(:,jj) + C1*u(:,jj) + C2*Utrans_ + D;
    % end
...
% er=y_initial_-XQ(:,1:end-1);
% ersq=er.*er;
% SSE_all = sum(ersq,'all')
% SSE = sum(ersq,2);
% MSE = SSE/size(y_initial_,2);
% RMSE = sqrt(MSE)
