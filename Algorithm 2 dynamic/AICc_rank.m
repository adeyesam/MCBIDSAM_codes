function [Objfun, Amat, C1mat, C2mat, Dmat] = AICc_rank(indx,iUY,i_UiUj,UYindex, Ncons)
    load('data4rank.mat','data_rank','u_rank','utrans_rank','mnd','mxd')
    data = data_rank;
    u = u_rank;
    Utrans = utrans_rank;
   ...
   while conv > tol
    ...
    %%
        ...
        if ~isempty(UYpresent)
            y(:,1:2) = data(:,1:2); 
            for ntr=3:lenreadpone
                [Utransi(:,ntr-1),~,~] = InputTransformation(y(:,ntr-1),u(:,ntr-1),iUY,i_UiUj,false);
                y(:,ntr) =  A*(y(:,ntr-1)-y(:,ntr-2)) + C1*u(:,ntr-1) + C2*Utransi(:,ntr-1) + D;
            end
        else
            for i = 2:lenreadpone
                y(:,i) =  A*y(:,i-1) + C1*u(:,i-1) + C2*Utrans(:,i-1) + D; 
        ...
        if ~isempty(UYpresent)
            ...
            for ntr=2:lenreadpone
                [Utransi(:,ntr-1),~,~] = InputTransformation(y(:,ntr-1),u(:,ntr-1),iUY,i_UiUj,false);
                y(:,ntr) =  A*y(:,ntr-1) + C1*u(:,ntr-1) + C2*Utransi(:,ntr-1) + D;
            end
        else
            for i = 2:lenreadpone
                y(:,i) =  A*y(:,i-1) + C1*u(:,i-1) + C2*Utrans(:,i-1) + D; 
