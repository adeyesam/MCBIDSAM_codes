function [Aic,Amat,C1mat,C2mat,Dmat,Lmat,Obtrnd] = BranchandBound(N,ndes,lengthu,Ntrans,Ncons,iUY,i_UiUj,UYindex,Nworker)
...
szlam = N^2 + (N-N^2)/2;
[Obj_base, Amat_b, C1mat_b, C2mat_b, Dmat_b, lmat_b] = ModelEstimation(0, iUY,i_UiUj,UYindex, Ncons, zeros(1,N2), zeros(1,N*lengthu),zeros(1,N*Ntrans),zeros(1,N),zeros(1,szlam));
...
ASTB=inf; AMATB=zeros(1,N2); C1MATB=zeros(1,N*lengthu); C2MATB=zeros(1,N*Ntrans); DMATB=zeros(1,N);  LMATB=zeros(1,szlam); OBTRNDB = zeros(1,Ntrans);
    ...
    function dumbsolve()
        while ~isempty(F)
            ...
            while NumberofNodes<Nworker && ~isempty(F)
                ...
                F(idc,:)=[];
                C(idc,:)=[];
                [a1,amat1,c1mat1,c2mat1,dmat1,lmat1]=feval(@Gc,f2,Ntrans,Ncons,lengthu,ASTB(idc,:),AMATB(idc,:),C1MATB(idc,:),C2MATB(idc,:),DMATB(idc,:),LMATB(idc,:),N,iUY,i_UiUj,UYindex,true,false)
                [a2,amat2,c1mat2,c2mat2,dmat2,lmat2]=feval(@Gc,f1,Ntrans,Ncons,lengthu,ASTB(idc,:),AMATB(idc,:),C1MATB(idc,:),C2MATB(idc,:),DMATB(idc,:),LMATB(idc,:),N,iUY,i_UiUj,UYindex,false,false)
            ...
            if ~isempty(F)  
                  ...
                  if ~isempty(problems(z).F)  
                    ...
                    while termin
                        ...
                        if flag1 && ~isempty(OBTRNDST_idn)
                              [a,amat,c1mat,c2mat,dmat, lmat]=feval(@Gc,f2,Ntrans,Ncons,lengthu,0,zeros(1,N2),zeros(1,N*lengthu),zeros(1,N*Ntrans),zeros(1,N),zeros(1,szlam),N,iUY,i_UiUj,UYindex,true,true);
                        ...
                        %Pruning of Branch 1
                        if (flag1 && cc>0)
                            [a,amat,c1mat,c2mat,dmat, lmat]=feval(@Gc,f2,Ntrans,Ncons,lengthu,AS,AMATS,C1MATS,C2MATS,DMATS,LMATS,N,iUY,i_UiUj,UYindex,true,false);
                            Abound=min([max(AT) max(DUMBA)]);
                            if a<AS 
                                AST=[AST ; a];
                                FT=[FT ; f2];
                    ...
                    SOLUTIONS(z).D4=C1MATT;
                    SOLUTIONS(z).D5=C2MATT;
                    SOLUTIONS(z).D6=DMATT;
                    SOLUTIONS(z).D7=LMATT;
                    SOLUTIONS(z).D8=OBTRNDT;
                  end  
                end
                ...
                for i=1:st
                    ...
                    C1mat=[C1mat ; SOLUTIONS(i).D4];
                    C2mat=[C2mat ; SOLUTIONS(i).D5];
                    Dmat=[Dmat ; SOLUTIONS(i).D6];
                    Lmat=[Lmat ; SOLUTIONS(i).D7];
                    Obtrnd=[Obtrnd ; SOLUTIONS(i).D8];
                end
            end
            if size(Aic,1)>ndes
                [Aic,idpd]=sort(Aic);
...
function [a,amat,c1mat,c2mat, dmat, lmat]=Gc(f,cv,Ncons,lengthu,as,amats,c1mats,c2mats,dmats,lmats,n,iUY,i_UiUj,UYindex,flag,flag2)
szlam = n^2 + (n-n^2)/2;
k=cv-sum(f);
warning('off', 'all')
if flag2
        [a, amat, c1mat, c2mat, dmat, lmat] = ModelEstimation(f,iUY,i_UiUj,UYindex,Ncons,amats,c1mats,c2mats,dmats,lmats);
else
    if k==cv
        a=inf; amat=zeros(1,n^2);c1mat=zeros(1,n*lengthu); c2mat=zeros(1,n*cv); dmat=zeros(1,n); lmat=zeros(1,szlam);
    elseif flag
        [a, amat, c1mat, c2mat, dmat, lmat] = ModelEstimation(f,iUY,i_UiUj,UYindex,Ncons,amats,c1mats,c2mats,dmats,lmats);
    else
        a=as; amat=amats; c1mat=c1mats; c2mat=c2mats; dmat=dmats; lmat=lmats;
    end
end
end