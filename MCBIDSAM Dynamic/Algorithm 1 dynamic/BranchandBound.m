function [Aic,Amat,Cmat,Lmat,Obtrnd] = BranchandBound(N,ndes,Ntrans,Ncons,iUY,i_UiUj,UYindex,Nworker)
    ...
    function dumbsolve()
        while ~isempty(F)
            ...
            while NumberofNodes<Nworker && ~isempty(F)
                ...
                F(idc,:)=[];
                C(idc,:)=[];
                [a1,amat1,cmat1,lmat1]=feval(@Gc,f2,Ntrans,ASTB(idc,:),AMATB(idc,:),CMATB(idc,:),LMATB(idc,:),N,iUY,i_UiUj,UYindex,Ncons,true,false);
                [a2,amat2,cmat2,lmat2]=feval(@Gc,f1,Ntrans,ASTB(idc,:),AMATB(idc,:),CMATB(idc,:),LMATB(idc,:),N,iUY,i_UiUj,UYindex,Ncons,false,false);
                  ...
                  if ~isempty(problems(z).F)  
                    ...
                    while termin
                        ...
                        if flag1 && ~isempty(OBTRNDST_idn)
                              [a,amat,cmat,lmat]=feval(@Gc,f2,Ntrans,0,zeros(1,N2),zeros(1,N*Ntrans),zeros(1,szlam),N,iUY,i_UiUj,UYindex,Ncons,true,true);
                        ...
                        %Pruning of Branch 1
                        if (flag1 && cc>0)
                            [a,amat,cmat,lmat]=feval(@Gc,f2,Ntrans,AS,AMATS,CMATS,LMATS,N,iUY,i_UiUj,UYindex,Ncons,true,false);
                            Abound=min([max(AT) max(DUMBA)]);
                            percent=(AS-a)/abs(AS)
                            if a<AS && percent>pct %Abound
                                AST=[AST ; a];
                                FT=[FT ; f2];
                                CT=[CT ; c1];
                                AMATST=[AMATST ; amat];
...
function [a,amat,cmat,lmat]=Gc(f,cv,as,amats,cmats,lmats,n,iUY,i_UiUj,UYindex,Ncons,flag,flag2)
szlam=n^2 + (n-n^2)/2;
k=cv-sum(f);
warning('off', 'all')
if flag2
        [a, amat, cmat, lmat] = ModelEstimation(f,iUY,i_UiUj,UYindex,Ncons,amats,cmats,lmats);
else
    if k==cv
