function [Obj_fun, A_mat, C_mat, l_mat, ybml_trend, yddr_trend, w_trend, fval_trend] = ConstrainedEstimation(indx, iUY, i_UiUj, UYindex, Ncons, Ain, Cin, Lin,nci,nhio)

load('data4est.mat','data_est','u_est', 'steady_indx', 'utrans_est','idOb','nut','mnd','mxd')
data = data_est;
u = u_est;
Utrans = utrans_est;
...
tic
    ...
    %% Constrained optimization of model parameters
        len = size(steady_indx,2);                                  % No. of steady state points
        objfun = @(x)ddr_obj(x, datas(:,2:end), ybmls(:,2:end), N, lenreading);
        nlcon = @(x)ddr_nlcon(x, AtomIN, N, Ncons, steady_indx, len, nci, nhio);
